...
for (i in unique(Fielding$playerID)) {
  ...
  Fielding2 <- rbind(Fielding2, data.frame(playerID = i, PO = sum(temp$PO), A = sum(temp$A), E = sum(temp$E), P = sum(temp[temp$POS == "P",]$G), C = sum(temp[temp$POS == "C",]$G), X1B = sum(temp[temp$POS == "1B",]$G), X2B = sum(temp[temp$POS == "2B",]$G), X3B = sum(temp[temp$POS == "3B",]$G), SS = sum(temp[temp$POS == "SS",]$G), OF = sum(temp[temp$POS == "OF",]$G)))
}

Fielding2$pitchPer <- Fielding2$P / (Fielding2$P + Fielding2$C + Fielding2$X1B + Fielding2$X2B + Fielding2$X3B + Fielding2$SS + Fielding2$OF)
Fielding2$batPer <- (Fielding2$C + Fielding2$X1B + Fielding2$X2B + Fielding2$X3B + Fielding2$SS + Fielding2$OF) / (Fielding2$P + Fielding2$C + Fielding2$X1B + Fielding2$X2B + Fielding2$X3B + Fielding2$SS + Fielding2$OF)
Fielding2 <- Fielding2 %>% select(playerID, PO, A, E, pitchPer, batPer)
...
for(i in unique(Appearances$playerID)) {
  ...
  apps <- rbind(apps, data.frame(playerID = i, team = j, P = sum(temp$G_p), C = sum(temp$G_c), X1B = sum(temp$G_1b), X2B = sum(temp$G_2b), X3B = sum(temp$G_3b), SS = sum(temp$G_ss), LF = sum(temp$G_lf), CF = sum(temp$G_cf), RF = sum(temp$G_rf), DH = sum(temp$G_dh), PH = sum(temp$G_ph), PR = sum(temp$G_pr)))
}
...
apps$position <- pos[apply(apps[,c(3:14)], 1, order)[12,]]
apps <- apps %>% select(playerID, team, position)
...
# Need: Gold Gloves, MVP Awards
awards <- AwardsPlayers %>% filter(awardID == "Gold Glove" | awardID == "Most Valuable Player")
awards2 <- data.frame()
for (i in unique(awards$playerID)) {
  temp <- awards %>% filter(playerID == i)
  awards2 <- rbind(awards2, data.frame(playerID = i, goldGloves = sum(temp$awardID == "Gold Glove"), MVPs = sum(temp$awardID == "Most Valuable Player")))
}
...
Master2 <- Master2 %>% dplyr::select(playerID, bbrefID.x, name, bats, throws, debut, finalGame, team, position, PO, A, E, rWAR, pitchPer, batPer, firstyearID.B, lastyearID.B, rWAR_batting, AB, H.B, HR.B, BB.B, G.B, AVG, R, RBI, SB, OBP, SLG, OPS, firstyearID.P, lastyearID.P, rWAR_pitching, W, L, ERA, G.P, GS, SV, IP, SO.P, WHIP, GPost, BAPost, WSW, WSL, goldGloves, MVPs, aStarApp, firstYearHOF, lastYearHOF, ballots, onBallot, inducted, votedBy, percentVote, banned)
colnames(Master2) <- c("playerID", "bbrefID", "name", "bats", "throws", "debut", "finalGame", "team", "position", "PO", "A", "E", "rWAR", "pitchPer", "batPer", "firstYearID.B", "lastYearID.B", "rWAR.B", "AB", "H", "HR", "BB", "G.B", "BA", "R", "RBI", "SB", "OBP", "SLG", "OPS", "firstYearID.P", "lastYearID.P", "rWAR.P", "W", "L", "ERA", "G.P", "GS", "SV", "IP", "SO", "WHIP", "GPost", "BAPost", "WSW", "WSL", "goldGloves", "MVPs", "allStarApps", "firstYearHOF", "lastYearHOF", "ballots", "onBallot", "inducted", "votedBy", "percentVote", "banned")
