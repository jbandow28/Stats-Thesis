...
HOF_batters <- baseball %>% filter(inducted & position != "P" & votedBy != "Negro League" & str_sub(playerID, -2, -1) != "99")

batters <- baseball %>% filter(position != "P" & votedBy != "Negro League" & str_sub(playerID, -2, -1) != "99" & AB >= low_out(HOF_batters$AB) & finalGame < as.Date("2004-01-01") & careerLength >= low_out(HOF_batters$careerLength) & rWAR >= low_out(HOF_batters$rWAR) & H >= low_out(HOF_batters$H) & R >= low_out(HOF_batters$R) & RBI >= low_out(HOF_batters$RBI) & G.B >= low_out(HOF_batters$G.B) & BA >= low_out(HOF_batters$BA) & OBP >= low_out(HOF_batters$OBP) & SLG >= low_out(HOF_batters$SLG))
